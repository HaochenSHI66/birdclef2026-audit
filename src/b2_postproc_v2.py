...
def _auc_col(yc, pc):
    """Vectorized rank-sum (Mann-Whitney) AUC for one class, tie-corrected via
    scipy.stats.rankdata (C-level, no python tie loop). == sklearn roc_auc_score."""
    n = len(yc)
    npos = yc.sum()
    if npos <= 0 or npos >= n:
        return np.nan
    ranks = _rankdata(pc)  # average ranks, handles ties
